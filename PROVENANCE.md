...
`createdAt` is the browser's local UTC timestamp. It has no external timestamp authority. The source hash is present only when the Studio can calculate or obtain it. The full production prompt is never embedded; only its SHA-256 digest is recorded.
